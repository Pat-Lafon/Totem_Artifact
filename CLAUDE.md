...
- Cobb and Cobb_Totem are separate git submodules — the user handles all submodule git operations themselves (init/update/status/commit/push). Edit files inside those directories freely; do not warn about, gate on, or prompt for submodule git workflow.
