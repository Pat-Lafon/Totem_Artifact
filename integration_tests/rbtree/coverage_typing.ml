...
let[@library] Rbtleaf = (is_rbtleaf v && num_black v 0 true : [%v: irbtree]) [@under]

let[@library] Rbtnode =
  let c = (true : [%v: bool]) [@over] in
  let lt = (true : [%v: irbtree]) [@over] in
  let x = (true : [%v: int]) [@over] in
  let rt = (true : [%v: irbtree]) [@over] in
  (is_rbtnode v && (color v) == c && (value v) == x && (left v) == lt && (right v) == rt
    : [%v: irbtree])
    [@under]
